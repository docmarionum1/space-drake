...
ig.baked=true;ig.module('game.settings').defines(function(){SCROLL_SPEED=16;WIDTH=512;HEIGHT=256;});

// lib/game/lib/entity.js
ig.baked=true;ig.module('game.lib.entity').requires('impact.entity','game.settings').defines(function(){ig.Entity.inject({update:function(){this.pos.x+=SCROLL_SPEED;if(this.flickerTimer>0){this.flickerTimer-=ig.system.tick;if(this.flickerTimer<=0){this.flickerTimer=0;this.isFlickering=false;}}
this.parent();},visible:function(){var screenCoords={left:ig.game.screen.x,top:ig.game.screen.y,right:ig.game.screen.x+ig.system.width,bottom:ig.game.screen.y+ig.system.height};var entityCoords={left:this.pos.x,top:this.pos.y,right:this.pos.x+this.size.x,bottom:this.pos.y+this.size.y};if((entityCoords.left>screenCoords.right)||(entityCoords.right<screenCoords.left)||(entityCoords.top>screenCoords.bottom)||(entityCoords.bottom<screenCoords.top)){return false;}else{return true;}},flickerTimer:0,isFlickering:false,flicker:function(duration){this.flickerTimer=duration;if(this.flickerTimer<=0){this.isFlickering=false;}},draw:function(){if(this.flickerTimer>0){this.isFlickering=!this.isFlickering;if(this.isFlickering===true){return;}}
this.parent();},circCollides:function(other){return(this.distanceTo(other)<(this.radius+other.radius));}});});

// lib/game/entities/bullet.js
ig.baked=true;ig.module('game.entities.bullet').requires('impact.entity','game.settings').defines(function(){EntityBullet=ig.Entity.extend({className:"bullet",size:{x:8,y:8},radius:4,checkAgainst:ig.Entity.TYPE.BOTH,type:ig.Entity.TYPE.B,animSheet:new ig.AnimationSheet('media/graphics/bullet.png',8,8),t:0,damage:5,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.maxVel.x=500;this.maxVel.y=500;},update:function(){this.cull();this.pattern();this.parent();},check:function(other){},cull:function(){if(!this.visible())
this.kill();}});});

// lib/game/entities/player.js
ig.baked=true;ig.module('game.entities.player').requires('impact.entity','game.settings','game.entities.bullet').defines(function(){EntityPlayer=ig.Entity.extend({className:"player",zIndex:10,size:{x:32,y:16},radius:1,checkAgainst:ig.Entity.TYPE.BOTH,type:ig.Entity.TYPE.A,animSheet:new ig.AnimationSheet('media/graphics/player.png',32,16),weaponDelay:new ig.Timer(.05),maxVel:{x:1000,y:1000},enableWeapons:true,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);},update:function(){velocity=192;if(ig.input.state('up'))
this.vel.y=-velocity;else if(ig.input.state('down'))
this.vel.y=velocity;else
this.vel.y=0;if(ig.input.state('left'))
this.vel.x=-velocity;else if(ig.input.state('right'))
this.vel.x=velocity;else
this.vel.x=0;if(this.enableWeapons&&ig.input.state('fire')&&this.weaponDelay.delta()>0){SOUNDS.laser.play();this.weaponDelay.reset();ig.game.spawnEntity(EntityBullet,this.pos.x+this.size.x,this.pos.y+4,{vel:{x:384,y:0},pattern:function(){}});ig.game.spawnEntity(EntityBullet,this.pos.x+this.size.x,this.pos.y+4,{vel:{x:384,y:-32},pattern:function(){}});ig.game.spawnEntity(EntityBullet,this.pos.x+this.size.x,this.pos.y+4,{vel:{x:384,y:32},pattern:function(){}});}
this.parent();if(this.pos.x<ig.game.screen.x)
this.pos.x=ig.game.screen.x;if(this.pos.y<ig.game.screen.y)
this.pos.y=ig.game.screen.y;if(this.pos.x+this.size.x>ig.game.screen.x+ig.system.width)
this.pos.x=ig.game.screen.x+ig.system.width-this.size.x;if(this.pos.y+this.size.y>ig.game.screen.y+ig.system.height)
this.pos.y=ig.game.screen.y+ig.system.height-this.size.y;},check:function(other){if(other.className=="sun"&&other.phase!="happy"&&this.circCollides(other)){this.kill();ig.game.gameOver=true;ig.game.restartDelay.reset();}
if(other.className=="enemyBullet"&&this.circCollides(other)){other.kill();this.kill();ig.game.gameOver=true;ig.game.restartDelay.reset();}}});});

// lib/game/entities/enemy.js
ig.baked=true;ig.module('game.entities.enemy').requires('impact.entity','game.settings').defines(function(){EntityEnemy=ig.Entity.extend({});});

// lib/game/entities/boss.js
ig.baked=true;ig.module('game.entities.boss').requires('game.entities.enemy','game.settings').defines(function(){EntityBoss=EntityEnemy.extend({});});

// lib/game/entities/enemybullet.js
ig.baked=true;ig.module('game.entities.enemybullet').requires('game.entities.bullet','game.settings').defines(function(){EntityEnemybullet=EntityBullet.extend({className:"enemyBullet",animSheet:new ig.AnimationSheet('media/graphics/bullets.png',8,8),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('fire',1,[1]);this.addAnim('tear',1,[2]);this.currentAnim=this.anims[this.currentAnim];}});});

// lib/game/entities/eyeball.js
ig.baked=true;ig.module('game.entities.eyeball').requires('game.entities.boss','game.settings','game.entities.enemybullet').defines(function(){EntityEyeball=EntityBoss.extend({size:{x:64,y:64},animSheet:new ig.AnimationSheet('media/graphics/eyeball.png',64,64),velocity:32,weaponDelay:new ig.Timer(.04),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.vel.y=-this.velocity;},update:function(){if((this.pos.y+this.size.y)>(ig.system.height-30)){this.vel.y=-this.velocity;}else if(this.pos.y<30){this.vel.y=this.velocity;}
if(this.weaponDelay.delta()>0){this.weaponDelay.reset();var r=14;var x=this.pos.x;var y=this.pos.y;ig.game.spawnEntity(EntityEnemybullet,this.pos.x+this.size.x/2-2,this.pos.y+this.size.y/2-2,{vel:{x:0,y:0},previous:{x:0,y:0},deltaT:15,pattern:function(){var new_x=r*this.t*Math.cos(this.t);var new_y=r*this.t*Math.sin(this.t);var delta_x=new_x-this.previous.x;var delta_y=new_y-this.previous.y;this.previous.x=new_x;this.previous.y=new_y;this.pos.x+=delta_x;this.pos.y+=delta_y;this.t+=this.deltaT*ig.system.tick;if(this.deltaT>1.5)
this.deltaT-=(this.deltaT-1.5)/100;},cull:function(){if(this.t>35)
this.kill();if(this.pos.x<ig.game.screen.x){this.kill();}}});ig.game.spawnEntity(EntityEnemybullet,this.pos.x+this.size.x/2-2,this.pos.y+this.size.y/2-2,{vel:{x:0,y:0},previous:{x:0,y:0},deltaT:15,pattern:function(){var new_x=r*this.t*Math.cos(this.t+Math.PI);var new_y=r*this.t*Math.sin(this.t+Math.PI);var delta_x=new_x-this.previous.x;var delta_y=new_y-this.previous.y;this.previous.x=new_x;this.previous.y=new_y;this.pos.x+=delta_x;this.pos.y+=delta_y;this.t+=this.deltaT*ig.system.tick;if(this.deltaT>1.5)
this.deltaT-=(this.deltaT-1.5)/100;},cull:function(){if(this.t>35)
this.kill();if(this.pos.x<ig.game.screen.x){this.kill();}}});}
this.parent();}});});

// lib/game/levels/test.js
ig.baked=true;ig.module('game.levels.test').requires('impact.image','game.entities.player','game.entities.eyeball').defines(function(){LevelTest={"entities":[{"type":"EntityPlayer","x":40,"y":184},{"type":"EntityEyeball","x":428,"y":132}],"layer":[{"name":"background3","width":2,"height":1,"linkWithCollision":false,"visible":1,"tilesetName":"media/graphics/backgrounds.png","repeat":true,"preRender":true,"distance":"4","tilesize":256,"foreground":false,"data":[[5,5]]},{"name":"background2","width":2,"height":1,"linkWithCollision":false,"visible":1,"tilesetName":"media/graphics/backgrounds.png","repeat":true,"preRender":true,"distance":"2","tilesize":256,"foreground":false,"data":[[3,3]]},{"name":"background1","width":2,"height":1,"linkWithCollision":false,"visible":1,"tilesetName":"media/graphics/backgrounds.png","repeat":true,"preRender":true,"distance":"1","tilesize":256,"foreground":false,"data":[[1,1]]}]};LevelTestResources=[new ig.Image('media/graphics/backgrounds.png'),new ig.Image('media/graphics/backgrounds.png'),new ig.Image('media/graphics/backgrounds.png')];});

// lib/game/levels/space.js
ig.baked=true;ig.module('game.levels.space').requires('impact.image','game.entities.player').defines(function(){LevelSpace={"entities":[{"type":"EntityPlayer","x":72,"y":124}],"layer":[{"name":"background0","width":8,"height":1,"linkWithCollision":false,"visible":1,"tilesetName":"media/graphics/spacebackground.png","repeat":true,"preRender":true,"distance":"4","tilesize":256,"foreground":false,"data":[[1,2,3,4,5,6,7,8]]},{"name":"background1","width":8,"height":1,"linkWithCollision":false,"visible":1,"tilesetName":"media/graphics/spacebackground.png","repeat":true,"preRender":true,"distance":"2","tilesize":256,"foreground":false,"data":[[9,10,11,12,13,14,15,16]]}]};LevelSpaceResources=[new ig.Image('media/graphics/spacebackground.png'),new ig.Image('media/graphics/spacebackground.png')];});

// lib/game/levels/title.js
ig.baked=true;ig.module('game.levels.title').requires('impact.image').defines(function(){LevelTitle={"entities":[],"layer":[{"name":"new_layer_0","width":2,"height":1,"linkWithCollision":false,"visible":true,"tilesetName":"media/graphics/splash.png","repeat":false,"preRender":false,"distance":"1","tilesize":256,"foreground":false,"data":[[1,2]]}]};LevelTitleResources=[new ig.Image('media/graphics/splash.png')];});

// lib/game/entities/sun.js
ig.baked=true;ig.module('game.entities.sun').requires('game.entities.boss','game.settings','game.entities.enemybullet').defines(function(){EntitySun=EntityBoss.extend({className:"sun",zIndex:-10,size:{x:128,y:128},radius:64,animSheet:new ig.AnimationSheet('media/graphics/sun.png',128,128),type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.BOTH,velocity:32,phase:"init",weaponDelay:new ig.Timer(.07),health:3000,invulnerable:true,timer:new ig.Timer(1),t:0,happy_counter:0,tears_y_max:64,angry_offset:0,init:function(x,y,settings){this.addAnim('happy',1,[0]);this.addAnim('happySad',.1,[0,1]);this.addAnim('sad',1,[1]);this.addAnim('sadAngry',.1,[1,2]);this.addAnim('angry',1,[2]);this.currentAnim=this.anims.happy;this.parent(x,y,settings);},update:function(){if(this.phase=="init"&&this.pos.x>ig.game.screen.x+(WIDTH-this.size.x*1.5)){this.vel.x=-this.velocity}else if(this.phase=="init"){this.phase="happy";this.timer.set(30);this.vel.x=0;ig.game.dialog([["Hi",1,function(){}],["Nice to meet you.",2,function(){this.invulnerable=false;}.bind(this)]]);}else if(this.phase=="happy"){if(this.timer.delta()>0){this.timer.set(30);if(this.happy_counter==0){ig.game.dialog([["I'm so glad that you're not attacking me.",3,function(){}]]);}else if(this.happy_counter==1){ig.game.dialog([["The last duck to visit started shooting me.",3,function(){}],["He mentioned something about pressing the SPACE BAR.",3,function(){}],["Whatever that means.  Trust me, I know space,",3,function(){}],["and have never seen any bars around here.",3,function(){}]]);}else if(this.happy_counter==2){ig.game.dialog([["...",1,function(){}]]);}else if(this.happy_counter==3){ig.game.dialog([["What's a duck like you doing out here anyway?",3,function(){}]]);}else if(this.happy_counter==4){ig.game.dialog([["Let's be friends ^_^",3,function(){}]]);}else if(this.happy_counter==5){ig.game.fadeScreen=true;ig.music.play('end');}
this.happy_counter++;}}else if(this.phase=="happySad"){if(this.timer.delta()>0){this.currentAnim=this.anims.sad;this.phase="sad";this.invulnerable=false;}}else if(this.phase=="sad"){if(this.weaponDelay.delta()>0){this.weaponDelay.reset();SOUNDS.shoot.play();this.tears_y=Math.cos(this.t)*this.tears_y_max;this.t+=.2;ig.game.spawnEntity(EntityEnemybullet,this.pos.x+75,this.pos.y+90,{vel:{x:-100,y:this.tears_y},currentAnim:"tear",pattern:function(){}});ig.game.spawnEntity(EntityEnemybullet,this.pos.x+35,this.pos.y+90,{vel:{x:-100,y:-this.tears_y},currentAnim:"tear",pattern:function(){}});}}else if(this.phase=="sadAngry"){if(this.timer.delta()>0){this.currentAnim=this.anims.angry;this.phase="angry";this.invulnerable=false;this.weaponDelay.set(1);}}else if(this.phase=="angry"){if(this.weaponDelay.delta()>0){this.weaponDelay.set(.1);var j=this.pos.x+60;var k=this.pos.y+60;var numBullets=32;var radius=64;var vel=3;for(var i=0;i<numBullets;i++){SOUNDS.shoot.play();var x=radius*Math.cos((i/numBullets)*2*Math.PI+this.angry_offset)+j;var y=radius*Math.sin((i/numBullets)*2*Math.PI+this.angry_offset)+k;var dx=x-j;var dy=y-k;ig.game.spawnEntity(EntityEnemybullet,x,y,{vel:{x:dx*vel,y:dy*vel},currentAnim:"fire",pattern:function(){}});}
this.angry_offset+=.015;}}else if(this.phase=="nova"){this.pos.x+=Math.random()*4-2;this.pos.y+=Math.random()*4-2;if(this.timer.delta()>0){this.kill();}}
this.parent();},receiveDamage:function(amount,from){if(!this.invulnerable){this.parent(amount,from);if(this.phase=="happy"){this.invulnerable=true;this.phase="happySad";this.flicker(1);this.currentAnim=this.anims.happySad;this.timer.set(1);ig.game.flushDialog();ig.game.dialog([["WHY WOULD YOU DO THAT? D:",3,function(){}]]);ig.music.play('battle');return;}else if(this.phase=="sad"){if(this.health<1500){this.phase="sadAngry";this.invulnerable=true;this.currentAnim=this.anims.sadAngry;this.timer.set(1);this.flicker(1);return;}else{this.flicker(.05);}}else if(this.phase=="angry"){this.flicker(.05);}}},check:function(other){if(other.className=="bullet"&&this.circCollides(other)){other.kill();this.receiveDamage(other.damage,other);}},kill:function(){if(this.phase=="angry"){this.phase="nova";this.invulnerable=true;this.timer.set(5);this.flicker(5);}else if(this.phase=="nova"){var j=this.pos.x+60;var k=this.pos.y+60;var numBullets=256;var radius=64;var vel=7;for(var i=0;i<numBullets;i++){var x=radius*Math.cos((i/numBullets)*2*Math.PI+this.angry_offset)+j;var y=radius*Math.sin((i/numBullets)*2*Math.PI+this.angry_offset)+k;var dx=x-j;var dy=y-k;ig.game.spawnEntity(EntityEnemybullet,x,y,{vel:{x:dx*vel,y:dy*vel},currentAnim:"fire",pattern:function(){}});}
SOUNDS.explode.play();SOUNDS.explode.play();this.parent();}}});});

// lib/plugins/screen-fader.js
ig.baked=true;ig.module('plugins.screen-fader').requires('impact.timer','impact.animation').defines(function(){ig.ScreenFader=ig.Class.extend({defaultOptions:{color:{r:0,g:0,b:0,a:1},fade:'in',speed:1,screenWidth:0,screenHeight:0,waitUntilLoaded:true,visible:true},init:function(options){this._setOptions(options);var isFadingIn=this.options.fade!='out';this._alpha=isFadingIn?0:1;this._alphaChange=isFadingIn?1:-1;if(this.options.tileImagePath){if(isNaN(this.options.tileWidth)){throw new Error("ScreenFader option for tileWidth is invalid");}else if(isNaN(this.options.tileHeight)){throw new Error("ScreenFader option for tileHeight is invalid");}
this._sheet=new ig.AnimationSheet(this.options.tileImagePath,this.options.tileWidth,this.options.tileHeight);this._anim=new ig.Animation(this._sheet,1.0,[0]);this._anim.alpha=this._alpha;}
if(!isNaN(this.options.delayBefore)){var delayTime=this.options.delayBefore<=0?0:this.options.delayBefore;if(delayTime>0){this.timerDelayBefore=new ig.Timer(delayTime);}}},draw:function(){if(this.timerDelayAfter&&this.timerDelayAfter.delta()>0){delete this.timerDelayAfter;this._callUserCallback();}
if(this.timerDelayBefore){if(this.timerDelayBefore.delta()<0){return;}else{delete this.timerDelayBefore;}}
if(!this.options.visible){return;}
if(!this.isFinished&&(!this._sheet||(this._sheet.image.loaded||!this.options.waitUntilLoaded))){this._fadeAlphaValue();}
if(this._alpha<=0){return;}
if(this._anim){this.drawImageTiledOnScreen();}else{this.drawColorOnScreen();}},drawImageTiledOnScreen:function(){var tileX=0,tileY=0,totalWidth=this.options.screenWidth,totalHeight=this.options.screenHeight;var tileWidth=this.options.tileWidth,tileHeight=this.options.tileHeight;while(tileY<totalHeight){tileX=0;while(tileX<totalWidth){this._anim.draw(tileX,tileY);tileX+=tileWidth;}
tileY+=tileHeight;}},drawColorOnScreen:function(){ig.system.clear(this.getColorCssValue());},getColorCssValue:function(rgbaObject){var color=rgbaObject||this.options.color;var a=((typeof color.a!='undefined')?color.a:1)*this._alpha;if(a<0){a=0;}else if(a>1){a=1;}
return'rgba('+color.r+','+color.g+','+color.b+','+a+')';},finish:function(){if(this.isFinished){return;}
if(this._alphaChange>0){this._alpha=1;}else{this._alpha=0;}
if(this._anim){this._anim.alpha=this._alpha;}
this.isFinished=true;if(typeof this.options.callback=='function'){var delayTime=isNaN(this.options.delayAfter)?0:this.options.delayAfter;if(delayTime>0){this.timerDelayAfter=new ig.Timer(delayTime);}else{this._callUserCallback();}}},_callUserCallback:function(){this.options.callback.call(this.options.context||(ig.ScreenFader.globalGameIsContext?ig.game:this));},_fadeAlphaValue:function(){this._alpha+=(this._alphaChange*this.options.speed*ig.system.tick*ig.ScreenFader.globalSpeedFactor);if((this._alphaChange>0&&this._alpha>=1)||(this._alphaChange<0&&this._alpha<=0)){this.finish();}
if(this._anim){this._anim.alpha=this._alpha;}},_setOptions:function(userOptions){this.options=ig.copy(this.defaultOptions);if(isNaN(this.options.screenWidth)||this.options.screenWidth<=0){this.options.screenWidth=ig.system.width;}
if(isNaN(this.options.screenHeight)||this.options.screenHeight<=0){this.options.screenHeight=ig.system.height;}
if(userOptions){ig.merge(this.options,userOptions);}},});ig.ScreenFader.globalSpeedFactor=2/3;ig.ScreenFader.globalGameIsContext=true;});

// lib/plugins/multi-color-font.js
ig.baked=true;ig.module('plugins.multi-color-font').requires('impact.font').defines(function(){ig.Font.inject({colors:{},defaultColor:null,init:function(path,colors){colors=typeof colors!=='undefined'?colors:["FFFFFF"];for(var i=0;i<colors.length;i++){this.colors[colors[i]]=null;}
this.defaultColor=colors[0];this.parent(path);},onload:function(ev){this._loadMetrics(this.data);this._loadColors(this.data);this.parent(ev);},draw:function(text,x,y,align,color){color=typeof color!=='undefined'?color:this.defaultColor;if(typeof(text)!='string'){text=text.toString();}
if(text.indexOf('\n')!==-1){var lines=text.split('\n');var lineHeight=this.height+this.lineSpacing;for(var i=0;i<lines.length;i++){this.draw(lines[i],x,y+i*lineHeight,align);}
return;}
if(align==ig.Font.ALIGN.RIGHT||align==ig.Font.ALIGN.CENTER){var width=this._widthForLine(text);x-=align==ig.Font.ALIGN.CENTER?width/2:width;}
if(this.alpha!==1){ig.system.context.globalAlpha=this.alpha;}
for(var i=0;i<text.length;i++){var c=text.charCodeAt(i);x+=this._drawChar(c-this.firstChar,x,y,color);}
if(this.alpha!==1){ig.system.context.globalAlpha=1;}
ig.Image.drawCount+=text.length;},_drawChar:function(c,targetX,targetY,color){if(!this.loaded||c<0||c>=this.indices.length){return 0;}
var scale=ig.system.scale;var charX=this.indices[c]*scale;var charY=0;var charWidth=this.widthMap[c]*scale;var charHeight=(this.height-2)*scale;ig.system.context.drawImage(this.colors[color].data,charX,charY,charWidth,charHeight,ig.system.getDrawPos(targetX),ig.system.getDrawPos(targetY),charWidth,charHeight);return this.widthMap[c]+this.letterSpacing;},_loadColors:function(image){var canvas=ig.$new('canvas');canvas.width=image.width;canvas.height=image.height;var ctx=canvas.getContext('2d');ctx.drawImage(image,0,0);var px=ctx.getImageData(0,0,canvas.width,canvas.height);for(var color in this.colors){var r=parseInt(color.substring(0,2),16);var g=parseInt(color.substring(2,4),16);var b=parseInt(color.substring(4,6),16);for(var i=0;i<px.data.length;i+=4){if(px.data[i+3]!=0){px.data[i]=r;px.data[i+1]=g;px.data[i+2]=b;}}
ctx.putImageData(px,0,0);this.colors[color]=new ig.Image(canvas.toDataURL());}}});});

// lib/game/audio.js
ig.baked=true;ig.module('game.audio').requires('impact.sound').defines(function(){ig.Sound.channels=2;MUSIC={title:new ig.Sound('media/audio/music/title.*'),battle:new ig.Sound('media/audio/music/battle.*'),space:new ig.Sound('media/audio/music/space.*'),end:new ig.Sound('media/audio/music/end.*')};loadMusic=function(){ig.music.loop=true;ig.music.add('media/audio/music/title.*','title');ig.music.add('media/audio/music/space.*','space');ig.music.add('media/audio/music/battle.*','battle');ig.music.add('media/audio/music/end.*','end');};SOUNDS={shoot:new ig.Sound('media/audio/sounds/shoot.*'),laser:new ig.Sound('media/audio/sounds/laser.*'),hurt:new ig.Sound('media/audio/sounds/hurt.*'),explode:new ig.Sound('media/audio/sounds/explode.*')};SOUNDS.laser.volume=.1;SOUNDS.shoot.volume=.2});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','impact.sound','game.settings','game.lib.entity','game.levels.test','game.levels.space','game.levels.title','game.entities.sun','game.entities.player','plugins.screen-fader','plugins.multi-color-font','game.audio').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/fonts/04b03.font_big.png',["FFFFFF"]),startDelay:new ig.Timer(1),restartDelay:new ig.Timer(2),delay:new ig.Timer(.1),nextEnemy:new ig.Timer(1),dialogTimer:new ig.Timer(1),gameOver:false,win:false,fadeScreen:false,title:true,dialogQueue:[],currentLine:null,enemies:[{e:EntitySun,spawned:false,defeated:false}],currentEnemy:0,init:function(){ig.game=this;ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.SPACE,'fire');this.loadLevel(LevelTitle);this.screenFader=new ig.ScreenFader({color:{r:224,g:102,b:255},callback:function(){this.win=true;this.getEntitiesByType(EntityPlayer)[0].enableWeapons=false;}});loadMusic();ig.music.play('title');},update:function(){if(this.gameOver&&ig.input.state('fire')&&this.restartDelay.delta()>0){this.loadLevel(LevelTitle);this.title=true;this.gameOver=false;this.enemies[0].spawned=false;this.enemies[0].defeated=false;this.flushDialog();ig.music.play('title');this.startDelay.reset();}
else if(this.title&&ig.input.state('fire')&&this.startDelay.delta()>0){this.loadLevel(LevelSpace);this.title=false;ig.music.play('space');}
else if(!this.title&&this.nextEnemy.delta()>0&&!this.enemies[this.currentEnemy].spawned){this.spawnEntity(this.enemies[this.currentEnemy].e,this.screen.x+WIDTH+128,64);this.enemies[this.currentEnemy].spawned=true;this.sortEntitiesDeferred();}
if(!this.title)
this.screen.x+=SCROLL_SPEED;this.parent();},dialog:function(lines){this.dialogQueue=this.dialogQueue.concat(lines);},flushDialog:function(){if(this.currentLine)
this.currentLine[2]();this.currentLine=null;this.dialogQueue=[];},drawDialog:function(){if(this.dialogTimer.delta()>0){if(this.currentLine){this.currentLine[2]();}
if(this.dialogQueue.length>0){this.currentLine=this.dialogQueue.shift();this.dialogTimer.set(this.currentLine[1]);}else{this.currentLine=null;}}
if(this.currentLine){ig.game.font.draw(this.currentLine[0],WIDTH/2,HEIGHT/8,ig.Font.ALIGN.CENTER);}},draw:function(){this.parent();if(this.fadeScreen){this.screenFader.draw();}
this.drawDialog();if(this.gameOver){ig.game.font.draw("Game Over",WIDTH/2,HEIGHT/2,ig.Font.ALIGN.CENTER);}else if(this.win){ig.game.font.draw("Friendship Achieved <3",WIDTH/2,HEIGHT/2,ig.Font.ALIGN.CENTER);}}});ig.main('#canvas',MyGame,120,WIDTH,HEIGHT,2);});